...
(function (window, undefined) {
    ...
    window.Asc.plugin.onTranslate = function (){
        document.getElementById('greeting').innerHTML = window.Asc.plugin.tr("Hi there! I am Wenyuan copilot, how can I help you today?");
        document.getElementById('chat-title').innerHTML = window.Asc.plugin.tr("Wenyuan Copilot Chat");
    ...
    function getContextMenuItems(options) {
        ...
        let settings = {
            ...
            items: [
                {
                    id: 'WenYuan Copilot',
                    text: generateText('WenYuan Copilot'),
                    items: []
                }
            ]
        }
    ...
    //WenYuan Copilot plugin Functions
    ...
    window.Asc.plugin.attachContextMenuClickEvent('onGenerateSlides', function () {
        window.Asc.plugin.executeMethod('GetSelectedText', null, function (topic) {
            ...
            wenyuanChatRequest(request, "You are a good PPT assistant, please help user to generate the outline according to the topic", false, config)
                .then(response => {
        ...
        function sendMessage() {
            ...
            if (message.trim() !== '') {
                ...
                if (hasKey) {
                    ...
                    const message2 = window.Asc.plugin.tr('Set Your WenYuan API Key first');
                    displayMessage(message2, 'ai-message');
                    conversationHistory.push({ role: 'assistant', content: 'Set Your WenYuan API Key first' });
                }
            }
        }
    ...
    function checkApiKey() {
        ...
        if (ApiKey) {
            ...
            hasKey = false;
            displayMessage(generateText('Set Your WenYuan API Key first'), 'assistant',true);
            console.log("Set Your WenYuan API Key first > translated", generateText('Set Your WenYuan API Key first'))
        }
    }
    ...
    function generateImageFromText(text) {
        const startMessage = window.Asc.plugin.tr('Wenyuan AI is generating');
        window.Asc.plugin.executeMethod('StartAction', ['Block', startMessage]);
        wenyuanImgRequest(text).then(response => {
    ...
    const reSend = () => {
        ...
        if (hasKey) {
            ...
            const message = window.Asc.plugin.tr("Set Your WenYuan API Key first")
    ...
    function createAIMessageFrame(pin=false) {
        ...
        //         <img src="resources/wenyuan-avatar.png">
        ...
        avatarImg.src = "resources/wenyuan-avatar.png";
    ...
    function chatRequest(request) {
        ...
        wenyuanChatRequest(prompt, systemMessage, true, config, abortController.signal)
            .then(reader => {
